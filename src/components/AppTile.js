...
class AppTile extends Component {
  render() {
    const { className, icon, label, launchApp, withS } = this.props;
    return (
      <div className={`app-tile-btn ${className}`}>
        <div className="app-tile-btn-title">{label}</div>
        <div className="app-tile-btn-body">
          <i
            className={`fa${withS ? "s" : ""} fa-${icon}`}
            aria-hidden="true"
          />
          <div className="app-tile-launch-btn" onClick={launchApp}>
            Lanuch
          </div>
        </div>
      </div>
...
AppTile.propTypes = {
  ...
  withS: PropTypes.bool.isRequired
};
